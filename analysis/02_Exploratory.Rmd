...
phyloseq::psmelt(x.1.0.species) %>%
  ...
  xlsx::write.xlsx(file="output/Deliverables/Exploratory/SpeciesAbundance_counts.xls")
phyloseq::psmelt(x.1.0.species.ra) %>%
  ...
  xlsx::write.xlsx(file="output/Deliverables/Exploratory/SpeciesAbundance_RA.xls")

phyloseq::psmelt(x.1.0.phylum) %>%
  # select(SampleID,Phylum,Especie,Sexo,Edad,Edad_2020,Edad_2021,Region,Code,Year,Abundance)%>%
  xlsx::write.xlsx(file="output/Deliverables/Exploratory/PhylumAbundance_counts.xls")
phyloseq::psmelt(x.1.0.phylum.ra) %>%
  # select(SampleID,Phylum,Especie,Sexo,Edad,Edad_2020,Edad_2021,Region,Code,Year,Abundance)%>%
  dplyr::rename(`Relative Abundance`=Abundance)%>%
  xlsx::write.xlsx(file="output/Deliverables/Exploratory/PhylumAbundance_RA.xls")

biomformat::write_biom(biomformat::make_biom(phyloseq::otu_table(x.1.0),phyloseq::sample_data(x.1.0),phyloseq::tax_table(x.1.0)),biom_file="output/Deliverables/Exploratory/ps_metaphlan3.biom")
```
...
file.remove("output/Deliverables/Exploratory/BetaDiversity/Adonis_BL.xls")
xlsx::write.xlsx(file = "output/Deliverables/Exploratory/BetaDiversity/Adonis_BL.xls",sheetName = "treatment",vegan::adonis2(x.6.0.response~treatment,data=x.6.0.explanatory))
### Perform ANOVA for non-NA in treatment
xlsx::write.xlsx(file = "output/Deliverables/Exploratory/BetaDiversity/Adonis_BL.xls",sheetName = "dry_wet",vegan::adonis2(x.6.0.response~dry_wet,data=x.6.0.explanatory),append=T)
xlsx::write.xlsx(file = "output/Deliverables/Exploratory/BetaDiversity/Adonis_BL.xls",sheetName = "neutered",vegan::adonis2(x.6.0.response~neutered,data=x.6.0.explanatory),append=T)
xlsx::write.xlsx(file = "output/Deliverables/AExploratory/BetaDiversity/donis_BL.xls",sheetName = "dog_name",vegan::adonis2(x.6.0.response~dog_name,data=x.6.0.explanatory),append=T)
...
pT<-p0 / p1 | p2 / p4
ggsave(pT,filename = "output/Deliverables/Exploratory/BetaDiversity/Ordination_BLSamples.pdf",width=12,height=12)
```

## Probiotic Detection

The aim of this subanalysis is to try to detect the bacterial species in the probiotic that is administered to the dogs. The probiotic is FortiFlora, by Purina. It contains *Enterococcus faecium SF68*.

*Deliverables: Spaghetti Plot showing evolution*

Results show that the probiotic straing is found in group A samples (product *848590-FR80-036-003*) after the administration of FortiFlora. The same species is foudn in a single sample at baseline in group B, although origin of these species can not be from FortiFlora since it had not beed administered at this point.

Nevertheless, abundance of Fortiflora species in group A M4 and M6 samples is very low, and only detectable in 2/5 samples in this group. We'll work, with the assumption that group "A"(848590-FR80-036-003) is the one receiving FortiFlora. This will need to be unblinded at some point downstream.

```{r Probiotic detection}
suppressMessages(require(dplyr))
suppressMessages(require(phyloseq))
...
p0<-metar::get_phyloseq(myMREObject,type="metaphlan") %>% 
  phyloseq::psmelt() %>% 
