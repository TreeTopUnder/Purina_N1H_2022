...
*Deliverables: Goal1/BetaDiversity/NMDS_Ordination_vs_Timepoint.pdf*

No effect of  (*Timepoint*) is observed or significant in the global composition of the gut microbiome of treated animals. Animal identity, however, explains 61% of the microbiome composition. This is expected in longitudinal data.
...
MaasLin method allows for the accounting of other variables in the test design.
...
### Species Level
input_data<-myMREObject@taxa@metaphlan@phyloseq %>% otu_table() %>% t()
...
### Genus Level
input_data<-myMREObject@taxa@metaphlan@phyloseq %>% tax_glom(.,taxrank="Genus") %>% otu_table() %>% t()
input_metadata<-metar::get_meta(myMREObject) %>% as.data.frame()
rownames(input_metadata)<-input_metadata$SampleID
...
```{r functional analysis chunk 1,include=TRUE,echo=F, message=FALSE, warning=FALSE, paged.print=FALSE, tidy=F,eval=F}
suppressMessages(require(dplyr))
suppressMessages(require(phyloseq))
suppressMessages(require(lefser))
suppressMessages(require(ggplot2))
...
suppressMessages(fit_data <- Maaslin2::Maaslin2(input_data=input_data,input_metadata=input_metadata,output="output/Deliverables/Goal1/DifferentialAbundance/maaslin/function/pathways",fixed_effects = c("Timepoint"),random_effects = c("Host_name"),cores = 4) )### No siginifcant hits



### Prepare input_data, 
input_data<-read.delim("data/raw/FunctionalAnnotation/HUMANN3/All_genefam_relab.tsv")
...
rownames(input_metadata)<-input_metadata$SampleID
rownames(input_metadata)
suppressMessages(fit_data <- Maaslin2::Maaslin2(input_data=input_data,input_metadata=input_metadata,output="output/Deliverables/Goal1/DifferentialAbundance/maaslin/function/genes",fixed_effects = c("Timepoint"),random_effects = c("Host_name"),cores = 4,min_prevalence = 0.4)) ### No siginifcant hits
